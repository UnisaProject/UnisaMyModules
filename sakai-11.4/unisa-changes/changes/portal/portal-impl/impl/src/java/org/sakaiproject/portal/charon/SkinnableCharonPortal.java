...
public class SkinnableCharonPortal extends HttpServlet implements Portal
{
			...
			Session session)
	{
		if (rcontext.uses(INCLUDE_LOGIN))
		{
			...
			// Unisa Changes:2019/03/12: Verify if user EID is for student or staff and create velocity context variables
			boolean isStudentLoginUserDispId = false;
			boolean isStaffLoginUserDispId = false;
			
			if( loginUserDispId.matches("[a-zA-Z].*") ) 
				isStaffLoginUserDispId = true;
			else
