 ...
 */
public class UnisaMessageHandlerFactory extends SakaiMessageHandlerFactory {

    private Logger log = LoggerFactory.getLogger(UnisaMessageHandlerFactory.class);
    ...
    public void init() {
        ...
        Objects.requireNonNull(authzGroupService, "AuthzGroupService must be set");
    ...
    public MessageHandler create(MessageContext ctx) {
        return new MessageHandler() {
            private String from;
            private boolean toStudent = false;
            private boolean toCourseSite = false;
            ...
            public void from(String from) throws RejectException {
            	if(isKnownByEmail(from)) {
            		this.from = from;
            	} else {
            		throw new RejectException(551, SENDERUNKNOWN + " - " + from);
            	}
            }
            ...
            public void recipient(String to) throws RejectException {
                ...
                if (serverConfigurationService.getServerName().equals(address.getDomain())) {
                    ...
                    // AT THIS POINT, DETERMINE THE ACTUAL DESTINATION MAILBOX
                    try {
                    	// FIRST ASSUME THAT IT IS A NEW EMAIL TO A SITE. IF IT IS A SITE WITH CONTACT EMAIL CONFIGURED...
                    	courseSite = getMailIdParts(address.getLocal(), RECIPIENT);
                    	site = siteService.getSite(courseSite);
                    	try {
                    		address = SplitEmailAddress.parse(site.getProperties().getProperty("contact-email").toLowerCase());
                    	} catch(NullPointerException npe) {
                    		log.info(courseSite + " does not have a contact-email property set. Cannot deliver mail to this module site.");
                    		throw new NullArgumentException(courseSite + " does not have a contact-email property set. Cannot deliver mail to this module site.");
                    	}
                    	recipient.address = address;
                    	log.debug("site configured with contact-email: " + recipient.getAddress());
                    	// DO NOT ADD RECIPIENT UNLESS YOU CAN PROVE (ROLE) MEMBERSHIP OF THE SITE
                    	if(studentNumber.equals("")) {
                    		studentNumber = SplitEmailAddress.parse(from).getLocal();
                    	}
                    	log.debug(studentNumber + " has USER ID of " + userDirectoryService.getUserByEid(studentNumber).getId());
                    	role = authzGroupService.getUserRole(userDirectoryService.getUserByEid(studentNumber).getId(), "/site/"+courseSite);
                    	if((null != role) && ("" != role)) {
                    		recipients.add(recipient);
                    		toCourseSite = true;
                    	} else {
                    		log.info("Email sender has no role ("+role+") in "+courseSite);
                    		throw new Exception("Email sender has no role in "+courseSite);
                    	}
                    } catch(IdUnusedException idue) {
                    	// LOOKS LIKE THE RECIPIENT IS PERHAPS A STUDENT THAT MUST RECEIVE EMAIL FROM A REPLYING LECTURER
                        try {
                        	...
                        	studentNumber = getMailIdParts(address.getLocal(), RECIPIENT);
                       		if(isStudent(studentNumber)) {
                        		...
                        		if(courseSite != "") {
                        			...
                        			recipients.add(recipient);
                        			toStudent = true;
                        			threadID = getMailIdParts(address.getLocal(), THREAD_ID);
                        		} else {
                        	...
                        	// SITE NOT FOUND
                        	throw new RejectException(551, SITENOTEXIST + " " + address.getLocal());
                        } catch(UserNotDefinedException unde) {
                        	// NO USER FOUND
                        	throw new RejectException(551, RCPTNOTEXIST + " " + address.getLocal());
                        } catch(Exception e) {
                        	throw new RejectException(551, e.getMessage());
                        }
                    } catch(IllegalArgumentException iae) {
                    	// NO CONTACT EMAIL CONFIGURED FOR THE SITE. COULD NOT PARSE. LOOKED FOR @ SYMBOL
                    	throw new RejectException(551, "Site " + getMailIdParts(address.getLocal(), RECIPIENT) + " has no contact-email property configured.");
                    } catch(Exception e) {
                    	e.printStackTrace();
                    	throw new RejectException(551, e.getMessage() + " - Cannot deliver email.");
                    }
                    //} else {
			//    log.info("No sending mail destined for " + to.toString());
		    //}
                } else {
            ...
            public boolean isStudent(String studentNumber) throws UserNotDefinedException {
            		User toUser = userDirectoryService.getUserByEid(studentNumber);
            ...
            public boolean isKnownByEmail(String email) {
        		if(userDirectoryService.findUsersByEmail(email).size() > 0) {
        			return true;
        		} else {
        			return false;
        		}
...
//                  Setup mail server
                   props.put("mail.smtp.host", serverConfigurationService.getString("smtp@org.sakaiproject.email.api.EmailService"));
                    ...
                    MimeMessage msg = new MimeMessage(Session.getDefaultInstance(props), data);
                    MimeMessage newmsg = new MimeMessage(Session.getDefaultInstance(props, null));
                    // Date can be null, need to fallback to better replacement
                    Date sent = msg.getSentDate();
                    if (sent == null) {
                        sent = new Date();
                        newmsg.setSentDate(sent);
                    ...
                    newmsg.setFrom(new InternetAddress(studentNumber+"."+courseSite+"."+threadID+"@"+serverConfigurationService.getServerName()));
                    InternetAddress[] replyAddresses = new InternetAddress[1];
                    replyAddresses[0] = new InternetAddress(studentNumber+"."+courseSite+"."+threadID+"@"+serverConfigurationService.getServerName());
                    newmsg.setReplyTo(replyAddresses);
                    newmsg.setContent(msg.getContent(),msg.getContentType());
                    newmsg.setContentID(msg.getContentID());
                    newmsg.setContentMD5(msg.getContentMD5());
                    newmsg.setDescription(msg.getDescription());
                    newmsg.setDisposition(msg.getDisposition());
                    newmsg.setFlags(msg.getFlags(), true);
...
/*                    Enumeration headers = msg.getAllHeaderLines();
                    ...
                    for (Recipient recipient : recipients) {
                        //archiveForRecipient(recipient, mailHeaders, subject, sent, id, msg);
                    	prepareAndSendEmail(newmsg, recipient);
                    }
            ...
            protected String getMailIdParts(String mailId, int part) {
                switch (part) {
                        case RECIPIENT:
                                if(mailId.indexOf('.') > 0) {
                                        log.debug("returning RECIPIENT as "+mailId.substring(0,mailId.indexOf('.')));
                                        return mailId.substring(0,mailId.indexOf('.'));
                                } else {
                                        log.debug("returning RECIPIENT as "+mailId);
                                        return mailId;
                                }
                        case SITE_NAME:
                                ...
                                if(mailId.indexOf('.') > 0) {
                                        // at least 2 parts
                                        // check 2nd part
                                        log.debug("mailId has at least 2 parts: "+mailId);
                                        String part2 = mailId.substring(mailId.indexOf('.')+1,mailId.length());
                                        if(part2.indexOf('.') > 0) {
                                                log.debug("mailId has 3 parts: "+mailId);
                                                // 3 parts ie includes thread id
                                                // return 2nd part
                                                String sid = part2.substring(0,part2.indexOf('.'));
                                                log.debug("returning SITE_NAME as sid "+sid);
                                                return sid;
                                        } else {
                                                // return 2nd part
                                                log.debug("returning SITE_NAME as "+part2);
                                                return part2;
                                        }
                                } else {
                                        return mailId;
                        ...
                        case THREAD_ID:
                                ...
                                if(mailId.indexOf('.') > 0) {
                                        // at least 2 parts
                                        // check 2nd part
                                        log.debug("mailId has at least 2 parts");
                                        String part2 = mailId.substring(mailId.indexOf('.')+1,mailId.length());
                                        if(part2.indexOf('.') > 0) {
                                                log.debug("mailId has 3 parts");
                                                // 3 parts ie includes thread id
                                                // return 2nd part
                                                String part3 = part2.substring(part2.indexOf('.')+1,part2.length());
                                                log.debug("returning SITE_NAME as "+part3);
                                                return part3;
                                        } else {
                                                return "";
                                        }
            ...
            protected void prepareAndSendEmail(MimeMessage msg, Recipient recipient) throws MessagingException {
                msg.setRecipients(javax.mail.Message.RecipientType.TO, recipient.getAddress());
                //msg.setSentDate(new Date());
                //msg.setFrom();
                log.debug("Sending email to "+recipient.getAddress()+" :"+msg);
                Transport.send(msg);
            }
            
            protected void archiveForRecipient(Recipient recipient, List<String> mailHeaders, String subject, Date sent, String id, MimeMessage msg) {
    ...
    protected class Recipient {
        ...
        protected String getAddress() {
        	return address.getLocal() + "@" + address.getDomain();
        }
    }
}
