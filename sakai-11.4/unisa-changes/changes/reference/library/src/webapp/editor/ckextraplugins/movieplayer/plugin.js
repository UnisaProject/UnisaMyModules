    ...
    CKEDITOR.dialog.add( 'movie.dlg', function( api ) {
            var dialogDef = {
                ...
                onOk : function() {
                    ...
                    var appendUrl ='';
                   // Unisa change: download option for videos which are uploaded from podcasts 
                    alert(tab1doc.getElementById('txtUrl').value);
                    if (tab1doc.getElementById('txtUrl').value.contains('Podcasts')) {
                             var appendUrl = '<BR> Click <a href="'+tab1doc.getElementById('txtUrl').value+'">here</a>to download the Video';
                           }
            
             this._.editor.insertHtml(fakeElement.getOuterHtml()+appendUrl);
