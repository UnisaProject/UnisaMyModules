...
public abstract class BaseUserDirectoryService implements UserDirectoryService, UserFactory
{
	...
	protected UserEdit getProvidedUserByEid(String id, String eid)
	{
		if (m_provider != null)
		{
			...
			if (m_provider.getUser(user))
			{
				...
				//unisa change: set type to student or lecturer or guest 
			      if (user.getEid().matches("[a-zA-Z].*")) {
	...
	public User getUser(String id) throws UserNotDefinedException
	{
		...
		if (user == null)
		{
			...
			}//unisa change: to add the type of the user if No type 
	...
	//unisa change: check the user from only DB not from AD (provider)
	public User getUserByEidFromDb(String eid) throws UserNotDefinedException
	{
		UserEdit user = null;
			...
			UserPermissionException
	{
		...
		//unisa change: check userId already exists in sakai_user_id_map for eid
		id =m_storage.checkMapForId(eid);
		...
		if(id==null){
		// unisa change: make sure we have an id //vijay: create userId for Eid
		...
		UserEdit user = m_storage.put(id, eid);  //unisa change: insert record into sakai_user_id_map and sakai_user
		if (user == null)
		{
			user = m_storage.getById(id);//get the user to update if aleready exists and not throw error
	...
	public class BaseUserEdit implements UserEdit, SessionBindingListener
	{
		...
		public String getDisplayName()
		{
			...
			if (rv == null)
			{
				...
				if (m_firstName != null) buf.append(m_firstName.trim().substring(0,1).toUpperCase()); //unisa change
				if (m_lastName != null)
				{
					if (buf.length() > 0) buf.append(" ");
					buf.append(m_lastName.trim().toUpperCase()); //unisa change
				}
