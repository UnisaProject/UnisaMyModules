...
public class PortletIFrame extends GenericPortlet {
 ...
 /** Choices of pixels displayed in the customization page */
    ...
    public String sanitizeHrefURL(String urlToEscape) {
         //return FormattedText.sanitizeHrefURL(urlToEscape); //mphahsm commented out
		 //Unisa-Changes: mphahsm - Applied the patch SAK-27791 - Allow spaces in URLs in the Web Content Tool 
    	String retval = FormattedText.sanitizeHrefURL(urlToEscape);
    	
    	int pos = retval.indexOf('?');
    	
    	if ( pos < 1 ) return retval;
    	
    	String firstBit = retval.substring(0,pos);
    	String secondBit = retval.substring(pos);
    	String secondBitTweaked = secondBit.replace("%20", "+");  // Try this with plus
    	
    	if ( secondBit.equals(secondBitTweaked) ) return retval;
    	
    	retval = firstBit + secondBitTweaked;
    	System.out.println("Web Content Tool Tweaked url="+retval);
    	return retval;
