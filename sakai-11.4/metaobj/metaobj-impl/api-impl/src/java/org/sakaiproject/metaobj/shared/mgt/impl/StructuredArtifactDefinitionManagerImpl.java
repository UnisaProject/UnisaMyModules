/**********************************************************************************
 * $URL: https://source.sakaiproject.org/svn/metaobj/tags/sakai-10.4/metaobj-impl/api-impl/src/java/org/sakaiproject/metaobj/shared/mgt/impl/StructuredArtifactDefinitionManagerImpl.java $
...
public class StructuredArtifactDefinitionManagerImpl extends HibernateDaoSupport
   ...
   public List findHomes(boolean includeGlobal, boolean includeHidden) {
      ...
      List sitesPulledFromService = getWorksiteManager().getUserSites();

      //NOTE: Previously, this code worked directly with the list pulled from the siteService directly.
      //      It would then remove elements from the list should the size exceed expressionMax (999 in this case)
      //      This would cause sites to "disappear" for a user - they would only see numSitesForUser % 999 sites.
      //      This way, the list is copied into another list so that it can be trimmed as necessary (done to prevent large changes to code)
      List sites = new ArrayList(sitesPulledFromService.size());
      sites.addAll(sitesPulledFromService);
   ...
   private List<StructuredArtifactDefinitionBean> findAllHomes() {
	   ...
	   return (List<StructuredArtifactDefinitionBean>)getHibernateTemplate().find(query);
   }
   ...
   public List<StructuredArtifactDefinitionBean> findAvailableHomes(Id currentWorksiteId, String currentUserId, boolean includeHidden, boolean includeGlobal) {
	   ...
	   Object[] params = new Object[]{new Integer(StructuredArtifactDefinitionBean.STATE_PUBLISHED),
               currentWorksiteId.getValue(),};
	   homes = (List<StructuredArtifactDefinitionBean>)getHibernateTemplate().findByNamedQuery(queryName, params);
