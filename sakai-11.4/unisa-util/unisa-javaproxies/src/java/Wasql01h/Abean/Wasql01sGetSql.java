 ...
 */
public class Wasql01sGetSql  implements ActionListener, java.io.Serializable  {
 
   //  Use final String for the bean name
   public static final String BEANNAME = new String("Wasql01sGetSql");
   ...
   private Wasql01sGetSqlOperation oper = null;
   ...
   public void execute()  throws PropertyVetoException {
      try  {
         if (oper == null) {
            oper = new Wasql01sGetSqlOperation(this);
         ...
         oper.doWasql01sGetSqlOperation();
   ...
   private class operListener implements ActionListener, java.io.Serializable  {
      private Wasql01sGetSql rP;
      operListener(Wasql01sGetSql r)  {
         rP = r;
      }
      public void actionPerformed(ActionEvent aEvent)  {
         if (Trace.isTracing(Trace.MASK_APPLICATION))
         {
            Trace.record(Trace.MASK_APPLICATION, "Wasql01sGetSql", "Listener heard that Wasql01sGetSqlOperation completed with " + 
   ...
   private void notifyCompletionListeners()  {
      if (Trace.isTracing(Trace.MASK_APPLICATION))
      {
         Trace.record(Trace.MASK_APPLICATION, BEANNAME, "notifying listeners of completion of operation Wasql01sGetSql ()\n");
      }
      Vector targets;
      ...
      actionEvt = new ActionEvent(this, 0, "Completion.Wasql01sGetSql");
   ...
   private void notifyExceptionListeners(String s)  {
      if (Trace.isTracing(Trace.MASK_APPLICATION))
      {
         Trace.record(Trace.MASK_APPLICATION, BEANNAME, "notifying listeners of exception of operation Wasql01sGetSql ()\n");
      }
      Vector targets;
      ActionEvent actionEvt = null;
      String failCommand = "Exception.Wasql01sGetSql";
      synchronized (this)  {
   ...
   public void clear()  throws PropertyVetoException  {
      ...
      importView.reset();
      exportView.reset();
      exportView.OutSqlGroup_MA = IntAttr.getDefaultValue();
   }

   Wasql01h.WASQL01S_IA importView = Wasql01h.WASQL01S_IA.getInstance();
   Wasql01h.WASQL01S_OA exportView = Wasql01h.WASQL01S_OA.getInstance();
   public String getInWsSqlQuerySqlId() {
      return StringAttr.valueOf(importView.InWsSqlQuerySqlId);
   }
   public void setInWsSqlQuerySqlId(String s)
      throws PropertyVetoException {
      if (s != null) {
          s = s.toUpperCase();
      }
      if (s.length() > 28) {
         throw new PropertyVetoException("InWsSqlQuerySqlId must be <= 28 characters.",
               new PropertyChangeEvent (this, "InWsSqlQuerySqlId", null, null));
      }
      importView.InWsSqlQuerySqlId = StringAttr.valueOf(s, (short)28);
   }
 
   public String getInWsSqlQueryDescription() {
      return StringAttr.valueOf(importView.InWsSqlQueryDescription);
   }
   public void setInWsSqlQueryDescription(String s)
      throws PropertyVetoException {
      if (s != null) {
          s = s.toUpperCase();
      }
      if (s.length() > 256) {
         throw new PropertyVetoException("InWsSqlQueryDescription must be <= 256 characters.",
               new PropertyChangeEvent (this, "InWsSqlQueryDescription", null, null));
      }
      importView.InWsSqlQueryDescription = StringAttr.valueOf(s, (short)256);
   }
   ...
   public final int OutSqlGroupMax = 10;
   public short getOutSqlGroupCount() {
      return (short)(exportView.OutSqlGroup_MA);
   };
 
   public String getOutGWsSqlQuerySqlId(int index) throws ArrayIndexOutOfBoundsException {
      if (9 < index || index < 0) {
         throw new ArrayIndexOutOfBoundsException("index range must be from 0 to 9, not: " + index);
      }
      return StringAttr.valueOf(exportView.OutGWsSqlQuerySqlId[index]);
   }
 
   public short getOutGWsSqlQuerySequenceNr(int index) throws ArrayIndexOutOfBoundsException {
      if (9 < index || index < 0) {
         throw new ArrayIndexOutOfBoundsException("index range must be from 0 to 9, not: " + index);
      }
      return exportView.OutGWsSqlQuerySequenceNr[index];
   }
 
   public String getOutGWsSqlQuerySql(int index) throws ArrayIndexOutOfBoundsException {
      if (9 < index || index < 0) {
         throw new ArrayIndexOutOfBoundsException("index range must be from 0 to 9, not: " + index);
      }
      return StringAttr.valueOf(exportView.OutGWsSqlQuerySql[index]);
   }
 
   public String getOutGWsSqlQueryDescription(int index) throws ArrayIndexOutOfBoundsException {
      if (9 < index || index < 0) {
         throw new ArrayIndexOutOfBoundsException("index range must be from 0 to 9, not: " + index);
      }
      return StringAttr.valueOf(exportView.OutGWsSqlQueryDescription[index]);
   }
 
   public String getOutGWsSqlQueryOwner(int index) throws ArrayIndexOutOfBoundsException {
      if (9 < index || index < 0) {
         throw new ArrayIndexOutOfBoundsException("index range must be from 0 to 9, not: " + index);
      }
      return StringAttr.valueOf(exportView.OutGWsSqlQueryOwner[index]);
   }
 
   public String getOutGWsSqlQuerySystemCode(int index) throws ArrayIndexOutOfBoundsException {
      if (9 < index || index < 0) {
         throw new ArrayIndexOutOfBoundsException("index range must be from 0 to 9, not: " + index);
      }
      return StringAttr.valueOf(exportView.OutGWsSqlQuerySystemCode[index]);
   }
 
   public Calendar getOutGWsSqlQueryDateChanged(int index) throws ArrayIndexOutOfBoundsException {
      if (9 < index || index < 0) {
         throw new ArrayIndexOutOfBoundsException("index range must be from 0 to 9, not: " + index);
      }
      return TimestampAttr.toCalendar(exportView.OutGWsSqlQueryDateChanged[index]);
   }
   public String getAsStringOutGWsSqlQueryDateChanged(int index) throws ArrayIndexOutOfBoundsException {
      if (9 < index || index < 0) {
         throw new ArrayIndexOutOfBoundsException("index range must be from 0 to 9, not: " + index);
      }
      return TimestampAttr.toString(exportView.OutGWsSqlQueryDateChanged[index]);
   }
