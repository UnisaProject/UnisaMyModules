 ...
 */
public class Saual60sGetUnqass  implements ActionListener, java.io.Serializable  {
 
   //  Use final String for the bean name
   public static final String BEANNAME = new String("Saual60sGetUnqass");
   ...
   private Saual60sGetUnqassOperation oper = null;
   ...
   public void execute()  throws PropertyVetoException {
      try  {
         if (oper == null) {
            oper = new Saual60sGetUnqassOperation(this);
         ...
         oper.doSaual60sGetUnqassOperation();
   ...
   private class operListener implements ActionListener, java.io.Serializable  {
      private Saual60sGetUnqass rP;
      operListener(Saual60sGetUnqass r)  {
         rP = r;
      }
      public void actionPerformed(ActionEvent aEvent)  {
         if (Trace.isTracing(Trace.MASK_APPLICATION))
         {
            Trace.record(Trace.MASK_APPLICATION, "Saual60sGetUnqass", "Listener heard that Saual60sGetUnqassOperation completed with " + 
   ...
   private void notifyCompletionListeners()  {
      if (Trace.isTracing(Trace.MASK_APPLICATION))
      {
         Trace.record(Trace.MASK_APPLICATION, BEANNAME, "notifying listeners of completion of operation Saual60sGetUnqass ()\n");
      }
      Vector targets;
      ...
      actionEvt = new ActionEvent(this, 0, "Completion.Saual60sGetUnqass");
   ...
   private void notifyExceptionListeners(String s)  {
      if (Trace.isTracing(Trace.MASK_APPLICATION))
      {
         Trace.record(Trace.MASK_APPLICATION, BEANNAME, "notifying listeners of exception of operation Saual60sGetUnqass ()\n");
      }
      Vector targets;
      ActionEvent actionEvt = null;
      String failCommand = "Exception.Saual60sGetUnqass";
      synchronized (this)  {
   ...
   public void clear()  throws PropertyVetoException  {
      ...
      importView.reset();
      exportView.reset();
      exportView.OutUnqassGrp_MA = IntAttr.getDefaultValue();
      for( int index = 0; index < 100; index++)
         exportView.OutGUniqueAssignmentType[index] = FixedStringAttr.valueOf("A", 1);
   }

   Saual60h.SAUAL50S_IA importView = Saual60h.SAUAL50S_IA.getInstance();
   Saual60h.SAUAL50S_OA exportView = Saual60h.SAUAL50S_OA.getInstance();
   ...
   public short getInWsStudentStudyUnitSemesterPeriod() {
      return importView.InWsStudentStudyUnitSemesterPeriod;
   }
   public void setInWsStudentStudyUnitSemesterPeriod(short s)
      throws PropertyVetoException {
      if (java.lang.Math.abs(s) >= 10.0) {
         throw new PropertyVetoException("InWsStudentStudyUnitSemesterPeriod has more than 1 digits.",
               new PropertyChangeEvent (this, "InWsStudentStudyUnitSemesterPeriod", null, null));
      }
      importView.InWsStudentStudyUnitSemesterPeriod = ShortAttr.valueOf(s);
   }
   public void setAsStringInWsStudentStudyUnitSemesterPeriod(String s)
      throws PropertyVetoException {
      if (s == null || s.length() == 0) {
          throw new PropertyVetoException("InWsStudentStudyUnitSemesterPeriod is not a valid numeric value: " + s,
                                          new PropertyChangeEvent (this, "InWsStudentStudyUnitSemesterPeriod", null, null));
      }
      try {
          setInWsStudentStudyUnitSemesterPeriod(Short.parseShort(s) );
      } catch (NumberFormatException e) {
          throw new PropertyVetoException("InWsStudentStudyUnitSemesterPeriod is not a valid numeric value: " + s,
                                          new PropertyChangeEvent (this, "InWsStudentStudyUnitSemesterPeriod", null, null));
      }
   ...
   public final int OutUnqassGrpMax = 100;
   public short getOutUnqassGrpCount() {
      return (short)(exportView.OutUnqassGrp_MA);
   };
 
   public short getOutGUniqueAssignmentYear(int index) throws ArrayIndexOutOfBoundsException {
      if (99 < index || index < 0) {
         throw new ArrayIndexOutOfBoundsException("index range must be from 0 to 99, not: " + index);
      }
      return exportView.OutGUniqueAssignmentYear[index];
   }
 
   public short getOutGUniqueAssignmentPeriod(int index) throws ArrayIndexOutOfBoundsException {
      if (99 < index || index < 0) {
         throw new ArrayIndexOutOfBoundsException("index range must be from 0 to 99, not: " + index);
      }
      return exportView.OutGUniqueAssignmentPeriod[index];
   }
 
   public int getOutGUniqueAssignmentUniqueNr(int index) throws ArrayIndexOutOfBoundsException {
      if (99 < index || index < 0) {
         throw new ArrayIndexOutOfBoundsException("index range must be from 0 to 99, not: " + index);
      }
      return exportView.OutGUniqueAssignmentUniqueNr[index];
   }
 
   public String getOutGUniqueAssignmentMkStudyUnitCode(int index) throws ArrayIndexOutOfBoundsException {
      if (99 < index || index < 0) {
         throw new ArrayIndexOutOfBoundsException("index range must be from 0 to 99, not: " + index);
      }
      return FixedStringAttr.valueOf(exportView.OutGUniqueAssignmentMkStudyUnitCode[index], 7);
   }
 
   public short getOutGUniqueAssignmentAssignmentNr(int index) throws ArrayIndexOutOfBoundsException {
      if (99 < index || index < 0) {
         throw new ArrayIndexOutOfBoundsException("index range must be from 0 to 99, not: " + index);
      }
      return exportView.OutGUniqueAssignmentAssignmentNr[index];
   }
 
   public short getOutGUniqueAssignmentNrOfQuestions(int index) throws ArrayIndexOutOfBoundsException {
      if (99 < index || index < 0) {
         throw new ArrayIndexOutOfBoundsException("index range must be from 0 to 99, not: " + index);
      }
      return exportView.OutGUniqueAssignmentNrOfQuestions[index];
   }
 
   public String getOutGUniqueAssignmentType(int index) throws ArrayIndexOutOfBoundsException {
      if (99 < index || index < 0) {
         throw new ArrayIndexOutOfBoundsException("index range must be from 0 to 99, not: " + index);
      }
      return FixedStringAttr.valueOf(exportView.OutGUniqueAssignmentType[index], 1);
   }
 
   public short getOutGUniqueAssignmentNegativeMarkFactor(int index) throws ArrayIndexOutOfBoundsException {
      if (99 < index || index < 0) {
         throw new ArrayIndexOutOfBoundsException("index range must be from 0 to 99, not: " + index);
      }
      return exportView.OutGUniqueAssignmentNegativeMarkFactor[index];
   }
 
   public short getOutGUniqueAssignmentMaxCredits(int index) throws ArrayIndexOutOfBoundsException {
      if (99 < index || index < 0) {
         throw new ArrayIndexOutOfBoundsException("index range must be from 0 to 99, not: " + index);
      }
      return exportView.OutGUniqueAssignmentMaxCredits[index];
   }
 
   public short getOutGUniqueAssignmentCreditSystem(int index) throws ArrayIndexOutOfBoundsException {
      if (99 < index || index < 0) {
         throw new ArrayIndexOutOfBoundsException("index range must be from 0 to 99, not: " + index);
      }
      return exportView.OutGUniqueAssignmentCreditSystem[index];
   }
 
   public short getOutGUniqueAssignmentPassMarkPercentage(int index) throws ArrayIndexOutOfBoundsException {
      if (99 < index || index < 0) {
         throw new ArrayIndexOutOfBoundsException("index range must be from 0 to 99, not: " + index);
      }
      return exportView.OutGUniqueAssignmentPassMarkPercentage[index];
   }
 
   public Calendar getOutGUniqueAssignmentClosingDate(int index) throws ArrayIndexOutOfBoundsException {
      if (99 < index || index < 0) {
         throw new ArrayIndexOutOfBoundsException("index range must be from 0 to 99, not: " + index);
      }
      return DateAttr.toCalendar(exportView.OutGUniqueAssignmentClosingDate[index]);
   }
   public int getAsIntOutGUniqueAssignmentClosingDate(int index) throws ArrayIndexOutOfBoundsException {
      if (99 < index || index < 0) {
         throw new ArrayIndexOutOfBoundsException("index range must be from 0 to 99, not: " + index);
      }
      return DateAttr.toInt(exportView.OutGUniqueAssignmentClosingDate[index]);
   }
 
   public String getOutGUniqueAssignmentCompulsory(int index) throws ArrayIndexOutOfBoundsException {
      if (99 < index || index < 0) {
         throw new ArrayIndexOutOfBoundsException("index range must be from 0 to 99, not: " + index);
      }
      return FixedStringAttr.valueOf(exportView.OutGUniqueAssignmentCompulsory[index], 1);
   }
 
   public int getOutGUniqueAssignmentTsaUniqueNr(int index) throws ArrayIndexOutOfBoundsException {
      if (99 < index || index < 0) {
         throw new ArrayIndexOutOfBoundsException("index range must be from 0 to 99, not: " + index);
      }
      return exportView.OutGUniqueAssignmentTsaUniqueNr[index];
   }
 
   public String getOutGYearMarkCalculationType(int index) throws ArrayIndexOutOfBoundsException {
      if (99 < index || index < 0) {
         throw new ArrayIndexOutOfBoundsException("index range must be from 0 to 99, not: " + index);
      }
      return FixedStringAttr.valueOf(exportView.OutGYearMarkCalculationType[index], 1);
   }
 
   public double getOutGYearMarkCalculationNormalWeight(int index) throws ArrayIndexOutOfBoundsException {
      if (99 < index || index < 0) {
         throw new ArrayIndexOutOfBoundsException("index range must be from 0 to 99, not: " + index);
      }
      return exportView.OutGYearMarkCalculationNormalWeight[index];
   }
 
   public String getOutGYearMarkCalculationOptionalityGc3(int index) throws ArrayIndexOutOfBoundsException {
      if (99 < index || index < 0) {
         throw new ArrayIndexOutOfBoundsException("index range must be from 0 to 99, not: " + index);
      }
      return StringAttr.valueOf(exportView.OutGYearMarkCalculationOptionalityGc3[index]);
   }
