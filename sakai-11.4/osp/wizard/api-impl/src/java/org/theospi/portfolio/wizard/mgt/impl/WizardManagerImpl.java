/**********************************************************************************
* $URL: https://source.sakaiproject.org/svn/osp/branches/sakai-10.x/wizard/api-impl/src/java/org/theospi/portfolio/wizard/mgt/impl/WizardManagerImpl.java $
...
public class WizardManagerImpl extends HibernateDaoSupport
   ...
   public Wizard getWizard(Id wizardId, int checkAuthz) {
      ...
      System.out.println("wizardmanaerimpl " + refs);
   ...
   public Node getNode(Id artifactId) {
      ...
      System.out.println("wizardmanager getnode " +                   getContentHosting().getReference(id));
...
*/
   ...
   // have to use raw sql because otherwise it will fetch full user data for owners of all osp_wizard_page records in order
   // to construct the full object.
   public List getWizardPagesByPageDefAndOwner(Id id, String owner) {
       Session sess = this.getSession();
       List wizards = null;
       try {
	   SQLQuery query = sess.createSQLQuery("select * from osp_wizard_page where wiz_page_def_id =? and owner=?");
	   wizards = query.addEntity(WizardPage.class)
	       .setString(0, id.getValue())
	       .setString(1, owner)
	       .list();
       } catch (Exception e) {
	   logger.error("getCompletedWizardPagesByPageDefAndOwner failed: " + e);
       }
   ...
   protected List getEvaluatableWizardPages(Agent agent, List<Agent> roles, List<String> worksiteIds, HashMap siteHash) {
      ...
      for (String siteid: worksiteIds) {
	  Site site = getWorksiteManager().getSite(siteid);
	  if (site != null) {
	      for (Object group: site.getGroupsWithMember(agent.getId().getValue()))
		  myGroups.add(((Group)group).getId());
	  }
      }

      List <String> wizPages = new ArrayList<String>();

      for ( Iterator it=wizardPages.iterator(); it.hasNext(); ) {
         EvaluationContentWrapper evalItem = (EvaluationContentWrapper)it.next();
	 wizPages.add(evalItem.getId().getValue());
      }

      // Rutgers hacks. Don't populate until needed, since in some cases they won't be.
      // old one: pages where access is limited by group
      Map<String,String> pagesGroups = null;
      // new one: pages where access is limited by instructor
      Map<String,Boolean> pagesByInstructor = null;
