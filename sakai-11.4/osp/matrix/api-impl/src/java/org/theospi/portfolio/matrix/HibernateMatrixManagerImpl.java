...
private static final String SCAFFOLDING_ID_TAG = "scaffoldingId";
   ...
   public List<Scaffolding> findPublishedScaffolding(String siteId) {
      ...
      return (List<Scaffolding>)getHibernateTemplate().find("from Scaffolding s where s.worksiteId=? " +
            "and s.published=?",
            params);
   }
   ...
   private void generateScaffoldingRealm(Scaffolding scaffolding) {
	   ...
	   try {
		   ...
		   templateAzg = authzGroupService.getAuthzGroup(realmTemplate + site.getType());
	   }
	   ...
	   try {
		   if (templateAzg != null) {
			   azg = authzGroupService.addAuthzGroup(
   ...
   public Node getNode(Id artifactId, boolean checkLocks) {
      ...
      System.out.println("hibernatematrix " +                getContentHosting().getReference(id));
   ...
   public void setAgentManager(AgentManager agentManager) {
    ...
    public void setAuthzGroupService (AuthzGroupService as) {
	this.authzGroupService = as;                                         
    }
   ...
   */
   ...
   public void checkPageAccess(String id) {
	  ...
	  if (this.getScaffoldingCellByWizardPageDef(wpd.getId()) != null) {
	  ...
	  if (this.getMatrixByPage(page.getId()) != null) {
      ...
      if (hasAccessThroughLink || canEval || canReview || owns || canAccessAllCells || canViewOtherReviews || canViewOtherEvals) {
         ...
         SecurityAdvisor contentAdvisor = (SecurityAdvisor)session.getAttribute("assignment.content.security.advisor");
            System.out.println("hibernatematrix contentadvisor " + contentAdvisor);
...
//       TODO Auto-generated catch block
   ...
   public List<WizardPageDefinition> getWizardPageDefs(List<Id> ids)
   {
	   if (ids.size() > 0) {
		   String[] paramNames = new String[] {"ids"};
		      ...
		      List<WizardPageDefinition> pageDefs = (List<WizardPageDefinition>)getHibernateTemplate().findByNamedParam("from WizardPageDefinition wpd where wpd.id in ( :ids )",
		            paramNames, params);
		   return pageDefs;
	   }
   ...
   public List<ScaffoldingCell> getScaffoldingCells(List<Id> ids)
   {
	   if (ids.size() > 0) {
		   String[] paramNames = new String[] {"ids"};
		      Object[] params = new Object[]{ids};
		      List<ScaffoldingCell> sCells = (List<ScaffoldingCell>)getHibernateTemplate().findByNamedParam("from ScaffoldingCell sCell where sCell.wizardPageDefinition.id in ( :ids )",
		            paramNames, params);
		   return sCells;
	   }
