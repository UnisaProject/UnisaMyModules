   ...
   protected void initBinder(HttpServletRequest request, ServletRequestDataBinder binder) {
	...
	protected int getTargetPage(HttpServletRequest request, Object command, Errors errors, int currentPage) {
      ...
      if (isFormSubmission(request)){
		...
		// we're having trouble with target 4 appearing spuriously along with something else
		// if we find anything else, use it. This code is copied from the Spring source.
		...
		while (paramNames.hasMoreElements()) {
			...
			int testVal = Integer.parseInt(paramName.substring(PARAM_TARGET.length()));
			if (retVal == 4 && testVal != 4)
			    return testVal;
		    }
		}
