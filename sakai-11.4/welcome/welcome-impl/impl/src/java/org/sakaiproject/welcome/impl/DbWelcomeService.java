	...
	protected class DbStorage implements Storage {
		...
		public String saveWelcomeContent(String siteId, String content) {
			...
			try {
				...
				if (count == 0) {
					sql = "insert into WELCOME_CONTENT(SITE_ID, CONTENT, MODIFIED_ON) values(?,?,to_date('"
							+ currentDate + "','YYYY-MM-DD'))";
					response = m_sqlService.dbWrite(sql, new Object[] { siteId, content });
				} else {
					sql = "update WELCOME_CONTENT" + " set CONTENT = ?, " + "     MODIFIED_ON = to_date(?,'YYYY-MM-DD')"
							+ " where SITE_ID = ?";
					sql = "update WELCOME_CONTENT" + " set CONTENT = ?, " + "     MODIFIED_ON = to_date('" + currentDate
							+ "','YYYY-MM-DD')" + " where SITE_ID = ?";
