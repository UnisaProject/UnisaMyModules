...
<%
 StudentUploadForm StudentUploadForm = (StudentUploadForm)session.getAttribute("StudentUploadForm");
%>
<sakai:html>
<head>
...
<html:form action="/studentUpload.do?act=upload" enctype="multipart/form-data" method="POST" target="_self">
         
	<html:hidden property="page" value="step1b"/>
	<input type="hidden" id="docMsg" name="docMsg" value="<bean:write name='studentUploadForm' property='webUploadMsg' />" />
	<input type="hidden" id="fileSelect" name="fileSelect" value="<bean:write name='studentUploadForm' property='fileSelected' />" />
	<input type="hidden" id="optionFile" name="optionFile" value="<bean:write name='studentUploadForm' property='optionFileBean.file' />" />
	<input type="hidden" id="hiddenButtonUpload" name="hiddenButtonUpload" value="<bean:write name='studentUploadForm' property='hiddenButtonUpload' />" />
	...
	<div class="container full-width">
			...
			<div class="panel panel-default">
				...
				<div class="panel-body">
						<table border="0" cellpadding="1" cellspacing="1" style="width:100%">
							...
							<logic:empty name="studentUploadForm" property="requiredDocs">
								<tr class="noBorder">
									<td colspan="4"><strong><fmt:message key="page.studentnr.upload.notrequired"/></strong>&nbsp;</td>
								</tr>
							</logic:empty>
							<logic:notEmpty name="studentUploadForm" property="requiredDocs">
								<tr class="noBorder">
								...
								<tr class="border">
								 ...
								 <logic:iterate name="studentUploadForm" property="requiredDocs" id="requiredDoc" indexId="currentIndex">
									<tr class="border">
										<td align="center">
										 	<logic:notEmpty name="studentUploadForm" property='<%="fileBeans["+currentIndex+"].uploaded"%>'>
										 		<label style="color:green"><strong><font size="large">Y</font></strong></label>
										 	</logic:notEmpty>
											<logic:empty name="studentUploadForm" property='<%="fileBeans["+currentIndex+"].uploaded"%>'>
												<label style="color:red"><strong><font size="large">N</font></strong></label>
											</logic:empty>
										</td>
										<td>				 
						       				&nbsp;<bean:write   name="requiredDoc" property="docDescription"/>
						       		 		<html:hidden name="studentUploadForm" property='<%="fileBeans["+currentIndex+"].doc.docCode"%>'/>
						       		 	</td>
										<td>
											<span style="float:left;"><html:file name="studentUploadForm" property='<%="fileBeans["+currentIndex+"].file"%>' maxlength="75" size="75px"></html:file></span>
											<span style="float:right;"><html:submit  property='<%="UploadReq["+currentIndex+"]"%>' value="Upload" onclick="needToConfirm = false;"/></span>
					       		 		</td>
										<td>
					        				 <logic:notEmpty name="studentUploadForm" property='<%="fileBeans["+currentIndex+"].uploaded"%>'>
					        				 		 <logic:iterate name="studentUploadForm" property='<%="fileBeans["+currentIndex+"].uploaded"%>' id="doc" indexId="docIndex">
							...
							<logic:notEmpty name="studentUploadForm" property="optionalDocs">
								<tr class="border">
							   		<td colspan="2">
							   			&nbsp;
							   			<html:select  name="studentUploadForm" property="optionFileBean.doc.docCode" value="-1">
							   		   		<html:option value="-1">Select From List&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</html:option>
											<html:options name="studentUploadForm" property="optionCodes"	labelProperty="optionLabels" />
										</html:select>
									</td>
									<td>
										<span style="float:left;"><html:file name="studentUploadForm" property="optionFileBean.file" maxlength="75" size="75px"></html:file></span>
									...
									<td>
										<div id="uploadedOptionalDocs">
											<logic:notEmpty name="studentUploadForm" property="desc">
												<logic:iterate name ="studentUploadForm" property='map' id="m" indexId="mIndex">
					...
					<div class="panel-footer clearfix">
						<logic:notEqual name="studentUploadForm" property="hiddenButtonUpload" value="true" >
		...
		<div style="display:none; align:center"><br><bean:write name="studentUploadForm" property="version"/></div>
