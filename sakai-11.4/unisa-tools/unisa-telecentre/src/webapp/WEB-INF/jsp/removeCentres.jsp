...
<sakai:html>
	<sakai:tool_bar>
		...
		<html:link href="telecentre.do?action=removeCentres" >
			<fmt:message key="profile.remove.link.selected"/>
		</html:link>
	...
	<sakai:messages/>
	<sakai:heading>
	     <fmt:message key="profile.remove.heading"/>                
	</sakai:heading>
	<html:form action="/telecentre.do">
	<logic:notEqual name="telecentreForm" property="telRemoveListTracker" value="1">
		<sakai:instruction>
		     <fmt:message key="profile.remove.instruction"/>                
		</sakai:instruction>
		<sakai:group_table>
			<tr>
		        <td><fmt:message key="telecentre.selectName"/></td>
			     <td>
			        <html:select name="telecentreForm" property="telecentreRemoveName" onchange="submit();">
					   <html:option value="-1">Select a Telecentre to Remove</html:option>
	...
	<logic:equal name="telecentreForm" property="telRemoveListTracker" value="1">
		<logic:equal name="telecentreForm" property="telecentreRemoveName" value="-1">
			<sakai:instruction>
			     <fmt:message key="profile.remove.instruction"/>                
			</sakai:instruction>
			<sakai:group_table>
	...
	<logic:equal name="telecentreForm" property="telRemoveListTracker" value="1">
		<logic:notEqual name="telecentreForm" property="telecentreRemoveName" value="-1">
			<sakai:instruction>
			     <fmt:message key="profile.remove.instruction2"/>                
			</sakai:instruction>
			<sakai:group_table>
				<sakai:group_heading>
					<fmt:message key="profile.remove.table.heading"/>
				</sakai:group_heading>
				<sakai:flat_list>
					<tr>
			    ...
			    <tr>
			    	<td>
			    		<sakai:actions>
			    			...
			    			<html:submit styleClass="button" property="action">
			    				<fmt:message key="profile.remove.submitBtn"/>
			    			</html:submit>
						</sakai:actions>
				 	</td>
				</tr>
	...
	<html:hidden property="action" value="handleRemoveListBoxAction"/>
	</html:form>
</sakai:html>
