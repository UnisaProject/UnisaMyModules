...
public class ForgotPasswordAction extends LookupDispatchAction {
	...
	public ActionForward studentnrstep (
			...
			eventTrackingService.post(
					eventTrackingService.newEvent(EventTrackingTypes.EVENT_PASSWORD_VIEW, null, false));
	 ...
	 * @throws Exception
	 */
	public String validateStudentNr(
			ActionMapping mapping,	
			ActionForm form,
			HttpServletRequest request,
			HttpServletResponse response) throws Exception {
			...
			/*joinActivationDAO is from shared classes used to validate
			  the student activation status*/
			...
			//IdvaltStudentDetailsDAO is from shared classes to validate student registration details  
		    ...
		    if (studentDetails.getStudentNr().equals("")) {		    	
		    	log.debug(this+": "+forgetPasswordForm.getStudentNr()+" not valid student number ");
		    	messages.add(ActionMessages.GLOBAL_MESSAGE,
				        new ActionMessage("message.generalmessage","Student number entered is not valid. Please enter a valid student number. "));
				addErrors(request, messages);
				eventTrackingService.post(
						eventTrackingService.newEvent(EventTrackingTypes.EVENT_PASSWORD_ERROR, "Student nr not valid", false), usageSession);
				return ("studnostep");
			}
		    }catch(Exception e){
				e.printStackTrace();
		    ...
		    try{
		    	 if(studentClaimedStatus==false) {
			    	log.debug(this+": "+forgetPasswordForm.getStudentNr()+" is not claimed unisa login ");
			    	messages.add(ActionMessages.GLOBAL_MESSAGE,
					        new ActionMessage("message.generalmessage","You do not have an active UNISA account. Please claim UNISA Login, by clicking on the appropriate link on the navigation menu."));
					addErrors(request, messages);
					eventTrackingService.post(
							eventTrackingService.newEvent(EventTrackingTypes.EVENT_PASSWORD_ERROR, "Student myLife account not created", false), usageSession);
					return ("studnostep");
			    }
			    }catch(Exception e){
			    	e.printStackTrace();
		    ...
		    if (joinActivationStuDetails.getStatus_flag().equals("BL")) {
		    	log.debug(this+": "+forgetPasswordForm.getStudentNr()+" is blocked.");
		    	messages.add(ActionMessages.GLOBAL_MESSAGE,
				        new ActionMessage("message.generalmessage","A new myUnisa password cannot be provided for this student number. "));
				addErrors(request, messages);
				eventTrackingService.post(
						eventTrackingService.newEvent(EventTrackingTypes.EVENT_PASSWORD_ERROR, "Student is blocked", false), usageSession);
				return ("studnostep");
		    }
		    ...
		    if(ckeckLastRequested==false){
		    	log.debug(this+": "+forgetPasswordForm.getStudentNr()+" trying password in less than an hour.");
		    	messages.add(ActionMessages.GLOBAL_MESSAGE,
				        new ActionMessage("message.generalmessage","You requested a new Unisa password less than an hour ago. Please wait a while to receive your SMS or try again later."));
				addErrors(request, messages);
				eventTrackingService.post(
						eventTrackingService.newEvent(EventTrackingTypes.EVENT_PASSWORD_ERROR, "Student trying password in less than an hour", false), usageSession);
				return ("studnostep");
		    }		    
	...
	public String validatePersonnelDetails(ActionMapping mapping,ActionForm form, HttpServletRequest request,HttpServletResponse response) throws Exception {
		ActionMessages messages = new ActionMessages(); 
		ForgetPasswordForm forgetPasswordForm = (ForgetPasswordForm) form;
		usageSessionService = (UsageSessionService) ComponentManager.get(UsageSessionService.class);
		UsageSession usageSession = usageSessionService.startSession(forgetPasswordForm.getStudentNr(), request.getRemoteAddr(), request.getHeader("user-agent"));
		...
		if (!messages.isEmpty()) {
		    ...
		    //tmp variables to select students Personal information from student database (proxt)
		    ...
		    // set the birth date
		    forgetPasswordForm.setDateOfBirth(forgetPasswordForm.getBirthYear()+"-"+forgetPasswordForm.getBirthMonth()+"-"+forgetPasswordForm.getBirthDay());
		   ...
		   if (!enteredSurname.equalsIgnoreCase(surnameInRecords)) {
				...
				eventTrackingService.post(
						eventTrackingService.newEvent(EventTrackingTypes.EVENT_PASSWORD_ERROR, "Surname doesn't match (User: "+enteredSurname.toUpperCase()+", System: "+surnameInRecords+")", false), usageSession);
		    ...
		    if (!enteredFullNames.equalsIgnoreCase(fullNamesInRecords)) {
				...
				eventTrackingService.post(
						eventTrackingService.newEvent(EventTrackingTypes.EVENT_PASSWORD_ERROR, "Full names don't match (User: "+enteredFullNames.toUpperCase()+", System: "+fullNamesInRecords+")", false), usageSession);
		    ...
		    if (!forgetPasswordForm.getDateOfBirth().equalsIgnoreCase(dateOfBirthInRecords)) {
				...
				eventTrackingService.post(
						eventTrackingService.newEvent(EventTrackingTypes.EVENT_PASSWORD_ERROR, "Date of birth doesn't match (User: "+forgetPasswordForm.getDateOfBirth()+", System: "+dateOfBirthInRecords+")", false), usageSession);
				return ("personneldetailstep");
		    }
		    ...
		    if ((enteredIdnr.length()>0)&&(!enteredIdnr.equalsIgnoreCase(idNumberInRecords))) {
				...
				eventTrackingService.post(
						eventTrackingService.newEvent(EventTrackingTypes.EVENT_PASSWORD_ERROR, "Id number doesn't match (User: "+enteredIdnr.toUpperCase()+", System: "+idNumberInRecords+")", false), usageSession);
				return ("personneldetailstep");
		    }
		    if ((enteredPassport.length()>0)&&(!enteredPassport.equalsIgnoreCase(passportNumberInRecords))) {
				...
				eventTrackingService.post(
						eventTrackingService.newEvent(EventTrackingTypes.EVENT_PASSWORD_ERROR, "Passport number doesn't match (User: "+enteredPassport.toUpperCase()+", System: "+passportNumberInRecords+")", false), usageSession);
				return ("personneldetailstep");
		 
			}			    
		    
		    eventTrackingService.post(
		    		eventTrackingService.newEvent(EventTrackingTypes.EVENT_PASSWORD_PERSDETAILSTEP, toolManager.getCurrentPlacement().getContext(), false), usageSession);
		   ...
		   /*  if the student cell number in the records is empty then student have to send a request to contact centre to update his cell number */
		    ...
		    //if the student is foreign student give an option to enter the email address to get his new password 
	...
	//validate email
	public String validateEmail(ActionMapping mapping, ActionForm form, HttpServletRequest request,HttpServletResponse response) throws Exception {
	 ...
	 private String procecssMyUnisaRequest(ForgetPasswordForm forgetPasswordForm,String stuNum,
		...
		/* The students who joined before 17 Jan 2013 are old students.
		   Implementation date will differentiates the two processes, before the implementation student can not see the initial
	  	   password and after the implementation of the new claim and forgot password we are checking the initial password changed field 
	  	   The new implementation does not deletes the Unisa_password records and no activation email sent
       */  
		...
		if (isOldStudent(Integer.parseInt(stuNum))) {
			/*
			 * if the student registered before 2009 (before myLife email
			 * process implementation) have no myLife email address and he can
			 * still use myUnisa account
			 */
			if (stuDao.getStudentLastAcadYear(stuNum) < 2009) {
				return forceNewPassword(forgetPasswordForm, stuNum);
			}
			// the student academic year from 2009 should have the active myLife email account
			if (forgetPasswordForm.getMyLifeActiveStatus().equals("Y")) {
				...
				messages.add(ActionMessages.GLOBAL_MESSAGE,new ActionMessage(
								"message.generalmessage","You have not claimed your UNISA Login yet. Please click on the appropriate link on the navigation menu to claim your Login."));
				addErrors(request, messages);
				return ("studnostep");
			}
		  }
			...
			/*
			 * if the student initial password changed then generate the new
			 * password else display the initial password on the screen
			 */
			if (initialPasswordChanged == true) {
				...
				eventTrackingService.post(eventTrackingService.newEvent(EventTrackingTypes.EVENT_PASSWORD_MYLIFEPWDVIEW,"Initial Unisa pwd viewed", false), usageSession);

				sendSms(forgetPasswordForm, forgetPasswordForm.getPassword(),request);
				eventTrackingService.post(eventTrackingService.newEvent("forgotpassword.send.sms", "SMS Unisa pwd to cell",false));
    ...
    private String emailError(ForgetPasswordForm forgetPasswordForm,
    		                  HttpServletRequest request,UsageSession usageSession){
                 			    
          try{
    	        ActionMessages messages = new ActionMessages(); 
    		    log.debug(this+": "+forgetPasswordForm.getStudentNr()+" email is not exist.");
	    	    messages.add(ActionMessages.GLOBAL_MESSAGE,
			        new ActionMessage("message.generalmessage","A new Unisa password cannot be provided for this student number. E-mail myUnisaHelp@unisa.ac.za with all your details and request assistance with your email address."));
			    addErrors(request, messages);
			    eventTrackingService.post(
					eventTrackingService.newEvent(EventTrackingTypes.EVENT_PASSWORD_ERROR, "Student email not exists", false), usageSession);
	       ...
	       eventTrackingService.post(
	    		eventTrackingService.newEvent(EventTrackingTypes.EVENT_PASSWORD_STUDENTNRSTEP, toolManager.getCurrentPlacement().getContext(), false), usageSession);
	       return ("studnostep");
    }
    ...
    public String processNoCellStudent(
			ActionMapping mapping,	
			ActionForm form,
			HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		    ...
		    if(message.length()==0){
		    	 messages.add(ActionMessages.GLOBAL_MESSAGE,
					        new ActionMessage("message.generalmessage","Message field may not be empty, please add a message."));
					    addErrors(request, messages);
			 return ("updateCellNumber");
		    }
		    if(altContact.length()==0){
		    	 messages.add(ActionMessages.GLOBAL_MESSAGE,
					        new ActionMessage("message.generalmessage","Contact details field may not be empty, please add a cell number or email address"));
					    addErrors(request, messages);
			 return ("updateCellNumber");
		    }
    		...
    		try {
    			...
    			try {
    				...
    				if(!saveStudentToAD.saveUser(idVaultuser)) {
    					...
    					log.error(this+" FORGOT PASSWORD:method:updatePassword  "+forgetPasswordForm.getStudentNr()+ "was not saved");
    					eventTrackingService.post(
    							eventTrackingService.newEvent(EventTrackingTypes.EVENT_PASSWORD_ERROR, "Save to provider failed", false), usageSession);
    					
    					messages.add(ActionMessages.GLOBAL_MESSAGE,
						        new ActionMessage("message.generalmessage", "Unfortunately we could not reset this student password, please try again."));
						addErrors(request, messages);
						return ("studnostep");
    				}
    				...
    				return ("studnostep");
    			     ...
    			     log.error(" Error occured on action=ForgotPasswordAction; method=updatePassword (UserNotDefinedException): set of password"+ e);
    		}
    		
     /*  update unisa_password table 
      * if student claimed unisa but password not changed 
      ...
      */
		    ...
		    sendUpdateCellNrEmail(forgetPasswordForm.getStudentNr(), selectRequest, message,altContact,forgetPasswordForm.getSurname(),
		    		forgetPasswordForm.getFullNames(),forgetPasswordForm.getDateOfBirth(),forgetPasswordForm.getId_nr(),forgetPasswordForm.getPassport_nr(),password);
		    eventTrackingService.post(
					eventTrackingService.newEvent("forgotpassword.send.email","email new UNISA pwd to contact center", false));
		    
    		}catch(Exception e){
    			 log.error("Error occured on action=ForgotPasswordAction; method=processNoCellStudent- send email failed"+ e);
    		}    		
			return "confirm";
	}
     ...
     public String confirm(
    			...
    			try {
    				...
    				if(!saveStudentToAD.saveUser(idVaultuser)) {
    					...
    					eventTrackingService.post(
    							eventTrackingService.newEvent(EventTrackingTypes.EVENT_PASSWORD_ERROR, "Save to provider failed", false), usageSession);    					
    					messages.add(ActionMessages.GLOBAL_MESSAGE,
						        new ActionMessage("message.generalmessage", "Unfortunately we could not reset your password, please try again."));
						addErrors(request, messages);
						return ("studnostep");
    				}
    			} catch (OperationNotSupportedException e) {
    				...
    				return ("studnostep");
    			     ...
    			     log.error(" Error occured on action=ForgotPasswordAction; method=confirm (UserNotDefinedException): set of password"+ e);
    		}

    		try {
    		//if the student changed the initial password using change password on  student workspace, update the unisa_password table 
    	    ...
    	    sendForgetPasswordEmail(studentNr,emailAddress,password);
    			...
    			log.error(" Error occured on action=ForgotPasswordAction; method=updatePassword(): Sending SMS/email failed"+ e);
    		}			
			return "confirm";
	}
	...
	public void sendForgetPasswordEmail(String stno, String emailAddress, String passwrd) throws AddressException {
		...
		String body="<html>" +
					   "<body>" +
					    ...
					    "Your new myUnisa user account details: <br> "+
					    "Student number : "+stno+"<br>"+
		...
		eventTrackingService.post(
				eventTrackingService.newEvent("forgotpassword.send.email","Send password to email "+emailAddress, false));
	...
	public void sendUpdateCellNrEmail(String stno, String selectRequest, String message,String altContact,
			String surName, String fullNames,String dob,String idNr,String passport,String newPassword) throws AddressException {
		...
		String body="<html>" +
					   "<body>" +
 					    "Dear myUnisa support team <br><br>"+
					    "The student below requested a New UNISA Password but there is no Cell number reflecting on our records.<br><br>"+
					    "Student Request:"+selectRequest+"<br>"+
					    "Student Message:"+message+"<br>"+
					    "Student <b>Alternative Contact details</b>:"+altContact+"<br><br>"+
					    "The student personnel details: <br> "+
					    "Student Number : "+stno+"<br>"+
					    "Student Surname : "+surName+"<br>"+
					    "Student Full names : "+fullNames+"<br>"+
					    "Student Date of Birth : "+dob+"<br>"+
					    "Student Id/passport number : "+idNr+" "+passport+"<br> <br>"+
					    "The new Password is <b>"+newPassword+"</b> <br><br>"+
					    "Please contact the student and inform them of their new UNISA password."+
					   "</body>"+
					  "</html>";
    ...
    private void sendSms(ForgetPasswordForm forgetPasswordForm,String password,HttpServletRequest request){
    	...
    	eventTrackingService = (EventTrackingService) ComponentManager.get(EventTrackingService.class);
    	UsageSession usageSession = usageSessionService.startSession(stuNum, request.getRemoteAddr(), request.getHeader("user-agent"));
	    request.getSession().setAttribute("UsageSession", usageSession);
    	...
    	try{
    	  cellNum=forgetPasswordForm.getCellNum().trim();
    	    if (cellNum.length()>= 1) { 
				...
				op.setInSmsQueueMessage("Confirmation: Your initial UNISA password was requested on "+date+". Your password is "+password);
				...
				if (op.getOutErrorFlagCsfStringsString1().equals("Y")) {
					eventTrackingService.post(
							eventTrackingService.newEvent("forgotpassword.intialpwd.view", "JavaProxy error: "+op.getOutCsfStringsString500()+ "("+cellNum+")", false), usageSession);
					    
										 
				}	   }
    	}catch(Exception ex){
    		 eventTrackingService.post(
					eventTrackingService.newEvent("forgotpassword.intialpwd.view", "There was an error sending sms : "+op.getOutCsfStringsString500()+ "("+cellNum+")", false), usageSession);	    

    	}
        eventTrackingService.post(
				eventTrackingService.newEvent("forgotpassword.send.sms","SMS new Unisa pwd to cell", false));  
    }
