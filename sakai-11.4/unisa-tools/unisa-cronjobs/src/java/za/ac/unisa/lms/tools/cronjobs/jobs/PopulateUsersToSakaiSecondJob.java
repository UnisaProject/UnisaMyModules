...
public class PopulateUsersToSakaiSecondJob extends SingleClusterInstanceJob implements StatefulJob, InterruptableJob, OutputGeneratingJob {
	...
	public void populateUsers() throws Exception{
		...
		PopulateUsersStudentSystemDAO populateUsersStudentSystemDAO = new PopulateUsersStudentSystemDAO();
		  ...
		  try{	
			  ...
			  userCourseList = populateUsersStudentSystemDAO.getDataFromIntSakFails();	//failures
			  ...
			  while (courseList.hasNext()) {					 
				 ...
				 String createdDate = courseDetails.get("CREATED_ON").toString();				 
					 ...
					 if(action.equalsIgnoreCase("DELETE")){
						 ...
						 populateUsersStudentSystemDAO.deleteUserFromFailuresTable(status,userId,userRole,keyData,tableName,action,fromValue,toValue,modifiedFlag,createdDate);
						 
					 }else if(action.equalsIgnoreCase("INSERT")){
						  checkStudentInSakai(userId); //check and add user to sakai
							...
							populateUsersStudentSystemDAO.deleteUserFromFailuresTable(status,userId,userRole,keyData,tableName,action,fromValue,toValue,modifiedFlag,createdDate); 						
								...
								String status =  addUserToSite(siteId,userId,accessLevel,userRole,keyData,tableName,action,fromValue,toValue,createdDate);
								 ...
								 if (modifiedFlag.equalsIgnoreCase("USER")){
									 ...
									 String removeStatus = removeUserToSite(siteId,fromValue,userRole,keyData,tableName,action,fromValue,toValue,createdDate);	
									 ...
									 checkUserInSakai(userId);
									 String removeStatus = removeUserToSite(siteId,userId,userRole,keyData,tableName,action,fromValue,toValue,createdDate);
