...
import za.ac.unisa.unisa_axis.DataCleanup_SamigoWorkingDataWebService_jws.DataCleanup_SamigoWorkingDataWebServiceServiceLocator;
import za.ac.unisa.unisa_axis.DataCleanup_SamigoWorkingDataWebService_jws.DataCleanup_SamigoWorkingDataWebService_PortType;

public class DataCleanupSamigoWorkingDataJob extends SingleClusterInstanceJob implements StatefulJob, OutputGeneratingJob {
...
+------------- min (0 - 59) (non-Javadoc)
	...
	public void executeLocked(JobExecutionContext context)
		...
		System.out.println("~~~~~~~~~~ Cronjob: DataCleanupSamigoWorkingDataJob");
		String subjectStart = "DataCleanupSamigoWorkingDataJob Job Started";
		String bodyStart = "DataCleanupSamigoWorkingDataJob Job Started";
		try {
		...
		url = serverUrl;
		url = url+"/unisa-axis/DataCleanup_SamigoWorkingDataWebService.jws";
		DataCleanup_SamigoWorkingDataWebService_PortType events = null;
		...
		for (int m=0; m < periods.length; m++) {
			...
			/** Delete old samigo working copies data */
			try {
				URL url1 = new URL(url);
				events = new DataCleanup_SamigoWorkingDataWebServiceServiceLocator().getDataCleanup_SamigoWorkingDataWebService(url1);	
			...
			try {
				...
				if(webServiceResult.trim().equals("Success")){
					subjectC = "DataCleanupSamigoWorkingDataJob Job Completed";
					bodyC = "DataCleanupSamigoWorkingDataJob Job Completed";
					System.out.println("~~~~~~~~~~ DataCleanupSamigoWorkingDataJob Job Completed");
				}
				else{
					subjectC = "DataCleanupSamigoWorkingDataJob Job FAILED";
					bodyC = "DataCleanupSamigoWorkingDataJob Job FAILED";
					System.out.println("~~~~~~~~~~ DataCleanupSamigoWorkingDataJob Job FAILED");
				}
...
} // end of public class DataCleanupSamigoWorkingDataJob