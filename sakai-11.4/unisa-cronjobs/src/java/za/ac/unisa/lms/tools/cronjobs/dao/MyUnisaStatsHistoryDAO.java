...
public class MyUnisaStatsHistoryDAO extends SakaiDAO {
	
	private UserDirectoryService userDirectoryService;

	/** history tables
	 * SAKAI_EVENT_HISTORY
	 * SAKAI_SESSION_HISTORY
	 */
	...
	public String getUserLoginDaily(String event, String forDate) throws Exception{
		...
		String select = "SELECT SAKAI_SESSION_HISTORY.SESSION_USER AS USR, COUNT(*) AS TOTAL "+
		             ...
		             "AND    TO_CHAR(SAKAI_EVENT_HISTORY.EVENT_DATE,'YYYY-MM-DD') = '"+forDate+"' "+
		             "GROUP BY SAKAI_SESSION_HISTORY.SESSION_USER ";
		...
		try{
    		...
    		while (j.hasNext()) {
    			...
    			try {
    				...
    				} else if (userTmp.getType().equals("lecturer")) {
    					lecturerLogin = lecturerLogin + total;
    				} else if (userTmp.getType().equals("student")) {
	...
	public String getUserLoginMonthly(String event, String forDate) throws Exception{
		...
		try{
    		...
    		errorMessage = insertUnisaMIS("user.login.unique",tmpDate+"-01",studentLogin,"gateway","S");
	...
	public String getUserLoginAnnual(String event, String forDate) throws Exception{
		...
		String select = "SELECT DISTINCT SAKAI_SESSION_HISTORY.SESSION_USER AS USR "+
						"FROM   SAKAI_SESSION_HISTORY "+
						"WHERE  TO_CHAR(SAKAI_SESSION_HISTORY.SESSION_START,'YYYY') = '"+forDate+"' "+
						"GROUP BY SAKAI_SESSION_HISTORY.SESSION_USER ";
	...
	public String getEventStatsMonthly(String event, String forDate, String dateType, String forSite, String userType) throws Exception{
		...
		String select = "SELECT COUNT(*) AS A "+
		             "FROM   SAKAI_EVENT_HISTORY "+
		             "WHERE  EVENT = '"+event+"' "+
		             "AND    TO_CHAR(EVENT_DATE,'YYYY-MM') = '"+forDate+"'";
...
/*	public void deleteEvent(String tmpDate,String action) throws Exception{
	...
	public void deleteEventsForMonth(String yearMonth) throws Exception{

		String sql1 = "DELETE FROM UNISA_MIS " +
					  "WHERE  TO_CHAR(DATE_COUNTED,'YYYY-MM') = '"+yearMonth+"' ";

		try{
			JdbcTemplate jdt1 = new JdbcTemplate(getDataSource());
			jdt1.update(sql1);
		} catch (Exception ex) {
			throw new Exception("MyUnisaStatsDAO: deleteEventsForMonth (Delete): Error occurred /"+ ex,ex);
		}
	}
	
	//public void deleteEventsForDay() throws Exception{
	public void deleteEventsForDay(String tmpDate) throws Exception{

		String sql1 = "DELETE FROM UNISA_MIS " +
					  "WHERE  TO_CHAR(DATE_COUNTED,'YYYY-MM-DD') = '"+tmpDate+"' ";
		...
		try{
			...
			throw new Exception("MyUnisaStatsDAO: deleteEventsForDay (Delete): Error occurred /"+ ex,ex);
		}
	}
